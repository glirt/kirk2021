...
import com.motun.kirk.Adapter.ChatUserAdapter;
import com.motun.kirk.Model.Chat;
import com.motun.kirk.Model.User;
import com.motun.kirk.Notification.Token;
...
//    }
    ...
    private void getEveryMessage() {
        final List<com.motun.kirk.Model.Chat> chats = new ArrayList<>();
        ...
        ref.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot dataSnapshot1 : dataSnapshot.getChildren()) {
                    com.motun.kirk.Model.Chat chat = dataSnapshot1.getValue(com.motun.kirk.Model.Chat.class);
                    if (Objects.requireNonNull(chat).getSender().equals(theIdToSearchFor) || chat.getReceiver().equals(theIdToSearchFor)) {
                        chats.add(chat);
                    }
                }

                for (final com.motun.kirk.Model.Chat chat1 : newRemoveRedundancies(chats)) {
    ...
    private List<com.motun.kirk.Model.Chat> arrangeChat(List<com.motun.kirk.Model.Chat> chats) {
        Collections.sort(chats, new Comparator<com.motun.kirk.Model.Chat>() {
            @Override
            public int compare(com.motun.kirk.Model.Chat o1, com.motun.kirk.Model.Chat o2) {
    ...
    private List<com.motun.kirk.Model.Chat> newRemoveRedundancies(List<com.motun.kirk.Model.Chat> result) {
        List<com.motun.kirk.Model.Chat> temporal = new ArrayList<>();
        for (com.motun.kirk.Model.Chat chat : result) {
            if (temporal.isEmpty()) {
                temporal.add(chat);
            } else {
